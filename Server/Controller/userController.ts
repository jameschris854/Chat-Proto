...
const removeFriend = async (req:Request,res:Response,next:NextFunction) : Promise<void> => {
  ...
  try{
    ...
    let isFriendPresent = userDoc.friends.find((e:any) => e._id == friendDoc._id)
    ...
    if(!isFriendPresent){
      ...
      currentFriends = currentFriends.filter((e:any) => e._id != friendDoc._id)

      let updatedUser = User.updateOne(friendDoc._id,{friends:currentFriends})
...
const updateFriend = async (req:Request,res:Response,next:NextFunction) : Promise<void> => {
  ...
  try{
    ...
    if(!IsNewFriend){
      ...
      let currentFriends : any[] = userDoc.friends;
      currentFriends.map((e) => {
        if(e._id === friendDoc._id){
          e.alias = aliasFriendName
          return e
        }else{
          return e
        }
