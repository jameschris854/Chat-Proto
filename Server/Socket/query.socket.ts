 ...
 */
const NotifyOnline = (members:SocketMembers,socket:AuthorizedSocket,userState:"ONLINE" | "OFFLINE") => {
    socket.handshake.jwtPayload.friends.forEach(friend => {
        if(members[`${friend._id}`]){
            io.of("v1/chat").to(members[`${friend._id}`].socketId).emit("USER_STATE_UPDATE",{id:socket.handshake.jwtPayload._id,status:userState})
        }
    })
}
 ...
 */
const sendMessage = async (req:AuthorizedHandShake,body:Body,callback: Function) : Promise< Function | AppError > => {
...
export default {queryConversationsByUser,sendMessage,forwardMessageToRecipient,NotifyOnline}